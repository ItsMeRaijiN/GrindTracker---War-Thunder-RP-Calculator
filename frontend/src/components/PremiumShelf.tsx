...
function formatInt(n: number | null | undefined) {
  return typeof n === 'number' ? n.toLocaleString('pl-PL') : '—'
}
...
export default function PremiumShelf({ items }: { items: Vehicle[] }) {
  const { premiums, collectors } = useMemo(() => {
    return {
      premiums: items.filter((v) => v.type === 'premium'),
      collectors: items.filter((v) => v.type === 'collector'),
    }
  }, [items])
